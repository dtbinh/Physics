...
Scene::Scene(GLWidget *parent)
{
    ...
    this->flag_scene = SCENE_POOL_OBJECTS;
...
//    }
    ...
    //for(int i=0;i<objects.size();i++)
        ...
        //Physics::closeObject(objects.at(i));


    Physics::initScene(this);
    if (flag_scene == SCENE_ARENA_OBJECTS)
        createArenaObjects();
    else if (flag_scene == SCENE_GROUND_ROTATIONAL)
        createGroundRotational();
    else if (flag_scene == SCENE_POOL_OBJECTS)
        createArenaPoolObjects();
...
//    }
    ...
    for(int i=0;i<this->sim_step;i++){
        if(characters.size()>0)
           ...
           if(elevator!=NULL){ //teste do elevador
               for(std::vector<ControlPD*>::iterator it = elevator->controllers.begin(); it!=elevator->controllers.end(); it++){
                   (*it)->evaluate();
               }
           }
...
std::vector<Object*> Scene::objectsScene()
{
    ...
    if(elevator!=NULL)
        for(unsigned int j=0;j<elevator->objects.size();j++) allobjetcs.push_back(elevator->objects.at(j));
...
std::vector<Joint*> Scene::jointsScene()
{
    ...
    if(elevator!=NULL)
        for(unsigned int j=0;j<elevator->joints.size();j++) alljoints.push_back(elevator->joints.at(j));
    return alljoints;
}
...
void Scene::setSceneConfiguration(int flag)
{
    flag_scene = flag;
}

int Scene::getSceneConfiguration()
{
    return flag_scene;
}
...
void Scene::createArena()
{
    ...
    //walls
    Object *wall_left = addObject(Vec4(lenght_wall,height_wall,(units+2)*dim_unit),offset+Vec4(-(units/2.)*dim_unit-lenght_wall/2.0,height_wall/2.0,0.0),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
    Object *wall_top = addObject(Vec4(units*dim_unit,height_wall,lenght_wall),offset+Vec4(0.0,height_wall/2.0,-6*dim_unit-lenght_wall),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
    Object *wall_right = addObject(Vec4(lenght_wall,height_wall,(units+2)*dim_unit),offset+Vec4((units/2.)*dim_unit+lenght_wall/2.0,height_wall/2.0,0.0),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
    Object *wall_bottom = addObject(Vec4(units*dim_unit,height_wall,lenght_wall),offset+Vec4(0.0,height_wall/2.0,6*dim_unit+lenght_wall),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
    ...
    joints.push_back(addJointFixed(wall_left,wall_right,NULL));
    joints.push_back(addJointFixed(wall_top,wall_bottom,NULL));

    //tijolos
    Vec4 dim_brick(0.05,0.05,0.01);
    float mass_brick = 10.5;
    int color_brick = MATERIAL_COPPER;
    //posição varia de acordo com a dimensão -units*dim_units<= x & y <= units*dim_units
         //posição varia de acordo com o quadrante -units/4.*dim_units<= x & y <= units/4.*dim_units
    //primeiro quadrante
   ...
   objects.push_back(box_7);
   objects.push_back(box_8);
   objects.push_back(box_9);
...
void Scene::createArenaObjects()
{
    Vec4 offset;

    float dim_unit = 0.3;

    float height_wall = 0.2; //altura
    float lenght_wall = 0.2; //largura
    float mass_wall = 500;
...
//    joints.push_back(addJointFixed(wall_top,wall_bottom,NULL));
    ...
    //tijolos
    Vec4 dim_brick(0.07,0.03,0.15);
    float mass_brick = 2.5;
    int color_brick = MATERIAL_COPPER;
    //posição varia de acordo com a dimensão -units*dim_units<= x & y <= units*dim_units
         //posição varia de acordo com o quadrante -units/4.*dim_units<= x & y <= units/4.*dim_units
    //primeiro quadrante
    ...
    //Object *brick1_1 = addObject(dim_brick,offset+Vec4(2*dim_unit,dim_brick.y()/2.0,5*dim_unit),QuaternionQ(Vec4(0,20,0)),TYPE_CUBE,mass_brick,0,color_brick);
    Object *brick1_1 = addObject(Vec4(dim_brick.x()/2,dim_brick.z(),dim_brick.x()/2),offset+Vec4(2*dim_unit,dim_brick.y()/2.0,5*dim_unit),QuaternionQ(Vec4(0,20,0)),TYPE_CYLINDER,mass_brick,0,MATERIAL_EMERALD);
    Object *brick2_1 = addObject(dim_brick,offset+Vec4(5*dim_unit,dim_brick.y()/2.0,4.5*dim_unit),QuaternionQ(Vec4(0,120,0)),TYPE_CUBE,mass_brick,0,color_brick);
    //Object *brick3_1 = addObject(dim_brick,offset+Vec4(5.5*dim_unit,dim_brick.y()/2.0,2.5*dim_unit),QuaternionQ(Vec4(0,120,0)),TYPE_CUBE,mass_brick,0,MATERIAL_EMERALD);
    Object *brick3_1 = addObject(Vec4(dim_brick.x()/2,dim_brick.z(),dim_brick.x()/2),offset+Vec4(5.5*dim_unit,dim_brick.y()/2.0,2.5*dim_unit),QuaternionQ(Vec4(0,30,0)),TYPE_CYLINDER,mass_brick,0,MATERIAL_EMERALD);
    Object *brick4_1 = addObject(dim_brick,offset+Vec4(1.5*dim_unit,dim_brick.y()/2.0,1.5*dim_unit),QuaternionQ(Vec4(0,-70,0)),TYPE_CUBE,mass_brick,0,color_brick);
    ...
    Object *brick1_4 = addObject(dim_brick,offset+Vec4(1.2*dim_unit,dim_brick.y()/2.0,-2.5*dim_unit),QuaternionQ(Vec4(0,20,0)),TYPE_CUBE,mass_brick,0,color_brick);
    Object *brick2_4 = addObject(dim_brick,offset+Vec4(0*dim_unit,dim_brick.y()/2.0,0*dim_unit),QuaternionQ(Vec4(0,120,0)),TYPE_CUBE,mass_brick,0,color_brick);
    //Object *brick2_4 = addObject(Vec4(dim_brick.x()/2,dim_brick.z(),dim_brick.x()/2),offset+Vec4(0*dim_unit,dim_brick.y()/2.0,0*dim_unit),QuaternionQ(Vec4(0,0,0)),TYPE_CYLINDER,mass_brick,0,MATERIAL_OBSIDIAN);
    //Object *brick2_4 = addObject(Vec4(dim_brick.x()/1.5,dim_brick.x()/1.5,dim_brick.x()/1.5),offset+Vec4(0*dim_unit,dim_brick.y()/2.0,0*dim_unit),QuaternionQ(Vec4(0,0,0)),TYPE_SPHERE,mass_brick,0,MATERIAL_OBSIDIAN);
    Object *brick3_4 = addObject(dim_brick,offset+Vec4(3.5*dim_unit,dim_brick.y()/2.0,-5.5*dim_unit),QuaternionQ(Vec4(0,30,0)),TYPE_CUBE,mass_brick,0,color_brick);
    Object *brick4_4 = addObject(dim_brick,offset+Vec4(2.0*dim_unit,dim_brick.y()/2.0,-3.0*dim_unit),QuaternionQ(Vec4(0,-70,0)),TYPE_CUBE,mass_brick,0,color_brick);
...
void Scene::createArenaPoolObjects()
{
    Vec4 offset;

    float dim_unit = 0.3;

    float height_wall = 0.1; //altura
    float lenght_wall = 0.1; //largura
    float mass_wall = 500;
    ...
    //walls
    Object *wall_left = addObject(Vec4(lenght_wall,height_wall,(units+1)*dim_unit),offset+Vec4(-(units/2.)*dim_unit-lenght_wall/2.0,height_wall/2.0,0.0),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
    Object *wall_top = addObject(Vec4(units*dim_unit,height_wall,lenght_wall),offset+Vec4(0.0,height_wall/2.0,-4.5/2*dim_unit-lenght_wall),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
    Object *wall_right = addObject(Vec4(lenght_wall,height_wall,(units+1)*dim_unit),offset+Vec4((units/2.)*dim_unit+lenght_wall/2.0,height_wall/2.0,0.0),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
    Object *wall_bottom = addObject(Vec4(units*dim_unit,height_wall,lenght_wall),offset+Vec4(0.0,height_wall/2.0,4.5/2*dim_unit+lenght_wall),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
...
//    joints.push_back(addJointFixed(wall_top,wall_bottom,NULL));
    ...
    //Object *brick1_1 = addObject(dim_brick,offset+Vec4(2*dim_unit,dim_brick.y()/2.0,5*dim_unit),QuaternionQ(Vec4(0,20,0)),TYPE_CUBE,mass_brick,0,color_brick);
    Object *brick1_1 = addObject(Vec4(dim_brick.x()/2,dim_brick.z(),dim_brick.x()/2),offset+Vec4(0.15,dim_brick.y()/2.0,0.15),QuaternionQ(Vec4(0,20,0)),TYPE_CYLINDER,mass_brick,0,MATERIAL_OBSIDIAN);
    Object *brick2_1 = addObject(Vec4(dim_brick.y(),dim_brick.y(),dim_brick.y()),offset+Vec4(0.,dim_brick.y()/2.0,0.),QuaternionQ(Vec4(0,120,0)),TYPE_SPHERE,mass_brick,0,MATERIAL_PEARL);
    //Object *brick3_1 = addObject(dim_brick,offset+Vec4(5.5*dim_unit,dim_brick.y()/2.0,2.5*dim_unit),QuaternionQ(Vec4(0,120,0)),TYPE_CUBE,mass_brick,0,MATERIAL_EMERALD);
    Object *brick3_1 = addObject(Vec4(dim_brick.x()/2,dim_brick.z(),dim_brick.x()/2),offset+Vec4(0.5,dim_brick.y()/2.0,0.5),QuaternionQ(Vec4(0,-30,0)),TYPE_CYLINDER,mass_brick,0,MATERIAL_OBSIDIAN);
    Object *brick4_1 = addObject(dim_brick,offset+Vec4(0.2,dim_brick.y()/2.0,0.4),QuaternionQ(Vec4(0,-70,0)),TYPE_CUBE,mass_brick,0,color_brick);

//    // segundo quadrante

    Object *brick1_2 = addObject(dim_brick,offset+Vec4(.1,dim_brick.y()/2.0,-.2*dim_unit),QuaternionQ(Vec4(0,-20,0)),TYPE_CUBE,mass_brick,0,color_brick);
    Object *brick2_2 = addObject(Vec4(dim_brick.y(),dim_brick.y(),dim_brick.y()),offset+Vec4(.3,dim_brick.y()/2.0,-1.5*dim_unit),QuaternionQ(Vec4(0,120,0)),TYPE_SPHERE,mass_brick,0,MATERIAL_PEARL);
    Object *brick3_2 = addObject(dim_brick,offset+Vec4(.5,dim_brick.y()/2.0,-0.2*dim_unit),QuaternionQ(Vec4(0,30,0)),TYPE_CUBE,mass_brick,0,color_brick);
    Object *brick4_2 = addObject(Vec4(dim_brick.y(),dim_brick.y(),dim_brick.y()),offset+Vec4(.15,dim_brick.y()/2.0,-0.6*dim_unit),QuaternionQ(Vec4(0,-70,0)),TYPE_SPHERE,mass_brick,0,MATERIAL_PEARL);

//    //terceiro quadrante

    Object *brick1_3 = addObject(Vec4(dim_brick.x()/2,dim_brick.z(),dim_brick.x()/2),offset+Vec4(-.2,dim_brick.y()/2.0,-.1),QuaternionQ(Vec4(0,90,0)),TYPE_CYLINDER,mass_brick,0,MATERIAL_OBSIDIAN);
    Object *brick2_3 = addObject(dim_brick,offset+Vec4(0,dim_brick.y()/2.0,-.6),QuaternionQ(Vec4(0,120,0)),TYPE_CUBE,mass_brick,0,color_brick);
    Object *brick3_3 = addObject(Vec4(dim_brick.y(),dim_brick.y(),dim_brick.y()),offset+Vec4(-.4,dim_brick.y()/2.0,-0.5),QuaternionQ(Vec4(0,30,0)),TYPE_SPHERE,mass_brick,0,MATERIAL_PEARL);
    Object *brick4_3 = addObject(dim_brick,offset+Vec4(-.5,dim_brick.y()/2.0,-.15),QuaternionQ(Vec4(0,-70,0)),TYPE_CUBE,mass_brick,0,color_brick);

//    //quarto quadrante

    Object *brick1_4 = addObject(dim_brick,offset+Vec4(-.2,dim_brick.y()/2.0,.5),QuaternionQ(Vec4(0,20,0)),TYPE_CUBE,mass_brick,0,color_brick);
    Object *brick2_4 = addObject(Vec4(dim_brick.y(),dim_brick.y(),dim_brick.y()),offset+Vec4(-.5,dim_brick.y()/2.0,0),QuaternionQ(Vec4(0,120,0)),TYPE_SPHERE,mass_brick,0,MATERIAL_PEARL);
    Object *brick3_4 = addObject(dim_brick,offset+Vec4(-.6,dim_brick.y()/2.0,.5),QuaternionQ(Vec4(0,30,0)),TYPE_CUBE,mass_brick,0,color_brick);
    Object *brick4_4 = addObject(Vec4(dim_brick.x()/2,dim_brick.z(),dim_brick.x()/2),offset+Vec4(-.1,dim_brick.y()/2.0,.3),QuaternionQ(Vec4(0,-70,0)),TYPE_CYLINDER,mass_brick,0,MATERIAL_OBSIDIAN);
...
void Scene::createGroundRotational()
{
    elevator = new Character(this);
    ...
    float height_wall = 0.2; //altura
    float lenght_wall = 0.2; //largura
    float mass_elevator = 100;

    int units = 12;

    int color_wall = MATERIAL_CHROME;

    Vec4 pos_anchor = ((Vec4(0,units/18.,0)+Vec4(0,0.1,0)+Vec4(0,0.05,0))+(Vec4(0,units/36.,0)+Vec4(0,0.1,0)+Vec4(0,units/36.0)))/2.;
    Vec4 pos_middle = (Vec4(0,units/18.,0)+Vec4(0,0.1,0)+Vec4(0,0.05,0))+((pos_anchor)-(Vec4(0,units/36.,0)+Vec4(0,0.1,0)+Vec4(0,units/36.0)));
    //walls
    Object *base_elevator = addObject(Vec4(units/4.,0.1,units/4.),Vec4(0,0.05,0),QuaternionQ(),TYPE_CUBE,mass_elevator/5.,elevator,MATERIAL_RUBY);
    Object *base_vertical_elevator = addObject(Vec4(units/54.,units/18.,units/54.),Vec4(0,units/36.,0)+Vec4(0,0.1,0),QuaternionQ(),TYPE_CUBE,mass_elevator/2.,elevator,color_wall);
    Object *top_elevator = addObject(Vec4(units/12.,0.1,units/12.),pos_middle,QuaternionQ(),TYPE_CUBE,mass_elevator/15.,elevator,MATERIAL_JADE);
//    Object *wall_top = addObject(Vec4(units*dim_unit,height_wall,lenght_wall),offset+Vec4(0.0,height_wall/2.0,-6*dim_unit-lenght_wall),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
//    Object *wall_right = addObject(Vec4(lenght_wall,height_wall,(units+2)*dim_unit),offset+Vec4((units/2.)*dim_unit+lenght_wall/2.0,height_wall/2.0,0.0),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);
//    Object *wall_bottom = addObject(Vec4(units*dim_unit,height_wall,lenght_wall),offset+Vec4(0.0,height_wall/2.0,6*dim_unit+lenght_wall),QuaternionQ(),TYPE_CUBE,mass_wall,0,color_wall);


    objects.push_back(base_elevator);
    objects.push_back(base_vertical_elevator);
    objects.push_back(top_elevator);
//    objects.push_back(wall_bottom);
    joints.push_back(addJointFixed(base_elevator,base_vertical_elevator,NULL));


    Joint *rotate_elevator = this->addJointBall(pos_anchor,base_vertical_elevator,top_elevator,elevator);
    joints.push_back(rotate_elevator);

    ControlPD *control = new ControlPD(rotate_elevator,QuaternionQ(1,0,0,0),Vec4(),Vec4());
    control->setKs(Vec4(6500.0,6500.0,6500.0));
    control->setKd(Vec4(1220.0,1220.0,1220.0));
    elevator->controllers.push_back(control);
...
void Scene::createCharacter()
{
    ...
    Object *A = addObject(Vec4(0.2,0.5,0.2),Vec4(0,0.90,0),QuaternionQ(1,0,0,0),TYPE_CUBE,3.0,chara,MATERIAL_CHROME);
    Object *B = addObject(Vec4(0.2,0.5,0.2),Vec4(0,0.35,0),QuaternionQ(1,0,0,0),TYPE_CUBE,2.5,chara,MATERIAL_EMERALD);
    A->setFoot(false);
    B->setFoot(false);
    ...
    ControlPD *pd = new ControlPD(joint,QuaternionQ(1,0,0,0),Vec4(),Vec4());
    pd->setEnabled(false);
...
void Scene::createLuxo()
{
    ...
    Object *lamp = addObject(Vec4(0.05,0.1,0.12),Vec4(0,0.8,0),QuaternionQ(1,0,0,0),TYPE_CUBE,0.5,luxo,materialLuxo);
    Object *upperBody = addObject(Vec4(0.05,0.2,0.12), Vec4(0,0.6,0),QuaternionQ(1,0,0,0),TYPE_CUBE,1,luxo,materialLuxo);
    Object *lowerBody = addObject(Vec4(0.05,0.3,0.12), Vec4(0,0.3,0), QuaternionQ(1,0,0,0),TYPE_CUBE,3,luxo,materialLuxo);
    Object *feet = addObject(Vec4(1.0,0.01,0.5), Vec4(0,0.05,0), QuaternionQ(1,0,0,0),TYPE_CUBE,4,luxo,materialLuxo);
    ...
    ControlPD *upperLampControl = new ControlPD(upperLamp,QuaternionQ(1,0,0,0),Vec4(),Vec4());
    ControlPD *lowerUpperControl = new ControlPD(lowerUpper,QuaternionQ(1,0,0,0),Vec4(),Vec4());
    ControlPD *feetLowerControl = new ControlPD(feetLower,QuaternionQ(1,0,0,0),Vec4(),Vec4());
    ...
    upperLampControl->setKd(Vec4(2.0,2.0,2.0));

    lowerUpperControl->setKs(Vec4(40.0,40.0,40.0));
    lowerUpperControl->setKd(Vec4(2.0,2.0,2.0));

    feetLowerControl->setKs(Vec4(40.0,40.0,40.0));
    feetLowerControl->setKd(Vec4(2.0,2.0,2.0));

    /*upperLampControl->setKs(Vec4(0.04,0.04,0.04));
    upperLampControl->setKd(Vec4(0.001,0.001,0.001));

    lowerUpperControl->setKs(Vec4(0.04,0.04,0.04));
    lowerUpperControl->setKd(Vec4(0.001,0.001,0.001));

    feetLowerControl->setKs(Vec4(0.05,0.05,0.05));
    feetLowerControl->setKd(Vec4(0.001,0.001,0.001));*/
    ...
    pose4a.push_back(Vec4(0,0,-30));
    pose4a.push_back(Vec4(0,0,60));
    Pose* pose4 = addPose(luxo,pose4a);
    pose4->setName("pose fim alternativa");

    GraphicalPose* luxoPose = addGraphicalPose(luxo);
    luxoPose->pushBackPose(pose1, 2000);
    luxoPose->pushBackPose(pose2, 2000);
    luxoPose->pushBackPose(pose3, 2000);

    luxoPose->setActive(true);
    luxoPose->setAdvancingTime(true);

    GraphicalPose* luxoPose2 = addGraphicalPose(luxo);
    luxoPose2->pushBackPose(pose1, 2000);
    luxoPose2->pushBackPose(pose2, 2000);
    luxoPose2->pushBackPose(pose4, 2000);
...
void Scene::updateAngleElevatorAdd(Vec4 angle)
{
    if(elevator==NULL) return;
    Vec4 atual = elevator->getController(0)->getEuler();
    elevator->getController(0)->setEuler(atual+angle);
    elevator->getController(0)->setQuaternionWanted(QuaternionQ(atual+angle));
